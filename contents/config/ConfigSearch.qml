...
KCM.SimpleKCM {
    ...
    readonly property var extendedRunners: [
        ...
        { id: "helprunner", name: i18nc("KRunner Plugin", "Help") },
        ...
        { id: "krunner_plasma-desktop", name: i18nc("KRunner Plugin", "Plasma Desktop") },
    ...
    // Combined and sorted list of known runners
    readonly property var knownRunners: {
        const all = [...essentialRunners, ...extendedRunners];
    ...
    Column {
        ...
        ListView {
            id: runnerListView
            model: cfg_searchRunners.concat(knownRunners.map((m) => m.id).filter((m) => !cfg_searchRunners.includes(m)))
            width: parent.width
